...
async def get_sse_stream() -> AsyncIterator[dict]:
    ...
    subscriptions = "unique.chat.external-module.chosen"

    url = (
        "https://api.genix.dev.echonet/public/event-socket/events/stream"
    ...
    while True:  # Infinite loop for automatic reconnection
        try:
            async with httpx.AsyncClient(
                timeout=300.0,
                trust_env=False,
                verify=settings.SSL_CA_CERT_PATH,
                ...
                async with client.stream("GET", url, headers=headers) as response:
                    ...
                    async for chunk in response.aiter_bytes():
                        ...
                        if time.time() - last_activity > 50:
                            logger.info(
                                "Proactive reconnection to avoid timeout"
                            )
                            break
...
async def start_sse_listener(
    ...
    logger.info(
        f"Starting SSE listener for: {settings.SUBSCRIPTIONS}"
    )
