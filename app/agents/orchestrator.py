    ...
    *,
    ...
    with operation_span(
        ...
        attributes={
            ...
            "app.turn.has_tool_outputs": bool(tool_outputs),
            "app.turn.history_present": bool(history_block.strip()),
        },
    ) as turn_span:
        ...
        try:
            ...
            with operation_span("agent.node.router", kind="CHAIN", attributes={"session.id": session_id}) as span:
                state.update(await router_agent(state))
                if span is not None:
                    span.set_attribute("app.route", state.get("route", ""))
                    span.set_attribute("app.execution_mode", state.get("execution_mode", ""))
            with operation_span("agent.node.query_details", kind="CHAIN", attributes={"session.id": session_id}) as span:
                state.update(
                ...
                if span is not None:
                    span.set_attribute("app.selected_portfolio_count", len(state.get("selected_portfolio_ids", [])))
            logger.info(
            ...
            if state.get("needs_portfolio_clarification") and not has_preloaded_data:
                ...
                if turn_span is not None:
                    turn_span.set_attribute("app.turn.outcome", "portfolio_clarification")
                return TurnResult(reply=question, citations=[], evaluations=[])

            if state.get("route") in DATA_ROUTES and not has_preloaded_data:
                with operation_span("agent.node.clarification", kind="CHAIN", attributes={"session.id": session_id}):
                ...
                if state.get("needs_clarification"):
                    ...
                    _persist_turn_state(session_id, message, state)
                    if turn_span is not None:
                        turn_span.set_attribute("app.turn.outcome", "clarification")
                    return TurnResult(reply=final_question, citations=[], evaluations=[])

            route = state.get("route", "")
            if route == ROUTE_OUT_OF_SCOPE:
                with operation_span("agent.node.synthesizer", kind="CHAIN", attributes={"session.id": session_id}):
            ...
            with operation_span("agent.node.evaluation", kind="CHAIN", attributes={"session.id": session_id}):
            ...
            if turn_span is not None:
                turn_span.set_attribute("app.route", state.get("route", ""))
                turn_span.set_attribute("app.execution_mode", state.get("execution_mode", ""))
                turn_span.set_attribute("app.turn.outcome", "completed")
                turn_span.set_attribute("app.citation_count", len(final_result.citations))
                turn_span.set_attribute("app.evaluation_count", len(final_result.evaluations))
            return final_result
        except Exception as exc:
            record_exception(turn_span, exc)
            if turn_span is not None:
                turn_span.set_attribute("app.turn.outcome", "failed")
            raise
