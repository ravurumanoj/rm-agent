...
class PortfolioInsightsAgent(BaseAgent):
    """Portfolio collector adapted from wealth app tool-calling pattern."""
    ...
    def _build_scoped_tool_args(
        ...
        if singular_keys:
            ...
            for portfolio_id in selected_portfolio_ids:
                ...
                scoped_calls.append((portfolio_id, scoped))
            return scoped_calls
    ...
    async def _apply_deterministic_fallback(
        ...
        logger.warning(
            "[PORTFOLIO] deterministic_fallback_triggered selected_ids=%s tools=%s",
        ...
        for tool in tools:
            ...
            logger.debug(
                "[PORTFOLIO] fallback_tool_call name=%s error=%s chunk_count=%s",
                name,
    ...
    async def collect_data(self, state: AgentState, *, extra_context: str = "") -> dict[str, Any]:
        injected = extract_injected_domain_results(
            ...
            default_tool_name="portfolio_tool",
            accepted_name_prefixes=("portfolio",),
        )
        if injected["tool_results"]:
            return injected

        metadata = state.get("metadata") or {}
        tools = metadata.get("portfolio_tools") or []
        if not isinstance(tools, list) or not tools:
            logger.warning("[PORTFOLIO] no_portfolio_tools_configured_in_metadata")
        ...
        logger.info(
            "[PORTFOLIO] collect_data_started selected_ids=%s available_tools=%s extra_context=%r",
            selected_portfolio_ids,
            [getattr(t, "name", "") for t in tools],
            extra_context[:200] if extra_context else "",
        )

        additional_context = "Portfolio-centric retrieval mode."
        if selected_portfolio_ids:
            additional_context += (
                "\nPortfolio IDs resolved from query: "
                + ", ".join(selected_portfolio_ids)
                + "\nCall portfolio tools only for these portfolio IDs."
            )
        if extra_context:
            additional_context += f"\n\n{extra_context}"

        prompt = PORTFOLIO_INSIGHTS_USER_TEMPLATE.format(
        ...
        for _ in range(max(1, settings.MCP_MAX_TOOL_ITERATIONS)):
            try:
                ...
                logger.warning("[PORTFOLIO] tool_planning_failed error=%s", exc)
                break
            if not getattr(ai, "tool_calls", None):
                logger.debug("[PORTFOLIO] llm_requested_no_tool_calls")
                break
            logger.debug(
                "[PORTFOLIO] llm_requested_tool_calls count=%s names=%s",
                len(ai.tool_calls),
            ...
            async def _call(tc: dict[str, Any]):
                ...
                if tool is None:
                    logger.warning("[PORTFOLIO] tool_not_found name=%s", name)
        ...
        logger.debug("[PORTFOLIO] effective_results_check result=%s tool_results_keys=%s", effective, list(tool_results.keys()))
        if not effective:
        ...
        logger.info(
            "[PORTFOLIO] collect_data_completed tools_called=%s chunk_count=%s",
            tools_called,
            len(chunks),
        )
