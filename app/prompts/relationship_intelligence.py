...
- Use only the relevant portfolio_id values, and retrieve only what the query needs.

Retrieval
- Call the tool(s) whose data the query needs; each tool's description states what it returns.
- Call independent tools in the same turn; pass the relevant portfolio_id to CRM tools.
- If portfolio IDs are provided in context and a CRM tool accepts portfolio_id/portfolio_ids, call it only for those IDs.
